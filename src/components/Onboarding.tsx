...
export function Onboarding() {
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('mindmesh_onboarded')) {
  ...
  const close = () => {
    localStorage.setItem('mindmesh_onboarded', '1');
    setOpen(false);
  };
      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
          ...
          <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0 }}
            ...
            <h3 className="mt-5 text-2xl font-bold gradient-text">Welcome to Mindmesh</h3>
            <p className="mt-2 text-sm text-muted-foreground">
              Hey, I'm Mindmesh AI. What's on your mind today? Press <kbd className="px-1.5 py-0.5 rounded bg-white/10 text-xs">⌘ K</kbd> any time to start a new chat.
            </p>
